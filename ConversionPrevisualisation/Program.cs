...
internal class DynamicObject
{
    public Dictionary<string, object> Properties { get; set; } =
        new(); // Dictionnaire pour stocker les propriétés dynamiques
}
...
internal class Program
{
        ...
        try
        {
            ...
            while (true) // boucle infinie
            {
                ...
                if (string.IsNullOrWhiteSpace(jsonFilePath) ||
                    !jsonFilePath.EndsWith(".json", StringComparison.OrdinalIgnoreCase))
                {
            ...
            var availableFields =
                objects.First().Properties.Keys.ToList(); // Obtenir les champs disponibles à partir du premier objet
            ...
            while (true)
            {
                ...
                if (!int.TryParse(fieldChoice, out var fieldIndex) || fieldIndex < 1 ||
                    fieldIndex > availableFields.Count) // Valider le choix
                {
            ...
            while (true)
            {
                ...
                for (var i = 0; i < availableFields.Count; i++) // Afficher les champs disponibles
                    Console.WriteLine($"{i + 1}-{availableFields[i]}");
                var sortChoice = Console.ReadLine();
                if (!int.TryParse(sortChoice, out var sortIndex) || sortIndex < 1 || sortIndex > availableFields.Count)
                {
                    ...
                    switch (exportChoice)
                    {
                        case "1":
                            ExportToCsv(finalResults, Path.Combine(exportDir, $"{fileName}.csv"),
                                availableFields); // Combine : chemin et nom de fichier
    ...
    private static IEnumerable<DynamicObject> FilterObjects(IEnumerable<DynamicObject> objects, string field,
        string value)
    {
        ...
        if (value.StartsWith(">"))
        {
            if (double.TryParse(value[1..], out var min))
                return objects.Where(o =>
                    o.Properties.TryGetValue(field, out var v) && double.TryParse(v?.ToString(), out var num) &&
                    num > min);
        }
        ...
        {
            return objects.Where(o =>
                o.Properties.TryGetValue(field, out var v) &&
                v?.ToString().Contains(value, StringComparison.OrdinalIgnoreCase) == true);
        }

        return objects;
    }

    private static IEnumerable<DynamicObject> SortObjects(IEnumerable<DynamicObject> objects, string field,
        bool descending)
    {
        return descending
            ? objects.OrderByDescending(o => o.Properties.TryGetValue(field, out var v) ? v?.ToString() : "")
            : objects.OrderBy(o =>
                o.Properties.TryGetValue(field, out var v) ? v?.ToString() : ""); // Gérer les valeurs nulles
    }
    ...
    private static void DisplayObjects(IEnumerable<DynamicObject> objects, List<string> fields)
    {
        ...
        foreach (var obj in results)
            Console.WriteLine(string.Join(", ",
                fields.Select(f => $"{f}: {obj.Properties.GetValueOrDefault(f)}"))); // Afficher toutes les propriétés
    }
    ...
    private static void ExportToCsv(List<DynamicObject> objects, string filePath, List<string> fields)
    {
        ...
        if (!exportAllFields)
        {
            ...
            fieldsToKeep = Console.ReadLine()
                .Split(',')
                .Select(f =>
                    int.TryParse(f.Trim(), out var idx) && idx >= 1 && idx <= fields.Count ? fields[idx - 1] : null)
                .Where(f => f != null)
                .ToList();
        }
        else
        ...
        } // fonction récuperée pour échapper les champs CSV
